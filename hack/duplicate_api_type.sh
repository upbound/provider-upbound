...
REPLACEMENT="$(printf '%s\n\npackage %s' "${DO_NOT_EDIT}" "${TO_PACKAGE}")"

echo "Replacing package line in ${FROM_PATH}..."
echo "From: package ${FROM_PACKAGE}"
echo "To  : ${REPLACEMENT}"
echo

sed "s#^package ${FROM_PACKAGE}\$#${REPLACEMENT}#" "${FROM_PATH}" > ${TO_PATH}
